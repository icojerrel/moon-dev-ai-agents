...
class ModelFactory:
    ...
    MODEL_IMPLEMENTATIONS = {
        "openrouter": OpenRouterModel,  # 🌟 RECOMMENDED - Access ALL models via one API key
        "claude": ClaudeModel,
        "groq": GroqModel,
        "openai": OpenAIModel,
    ...
    DEFAULT_MODELS = {
        "openrouter": "anthropic/claude-3.5-haiku",  # 🌟 Fast, cheap, and smart via OpenRouter
    ...
    def _initialize_models(self):
        ...
        for key in ["OPENROUTER_KEY", "GROQ_API_KEY", "OPENAI_KEY", "ANTHROPIC_KEY", "DEEPSEEK_KEY", "GROK_API_KEY"]:  # GEMINI_KEY temporarily removed
            value = os.getenv(key)
            if value and len(value.strip()) > 0:
                status = "✅ RECOMMENDED" if key == "OPENROUTER_KEY" else "✓"
                cprint(f"  ├─ {key}: Found ({len(value)} chars) {status}", "green")
            else:
                warning = "⚠️ HIGHLY RECOMMENDED!" if key == "OPENROUTER_KEY" else ""
                cprint(f"  ├─ {key}: Not found or empty {warning}", "red")
    ...
    def _get_api_key_mapping(self) -> Dict[str, str]:
        ...
        return {
            "openrouter": "OPENROUTER_KEY",  # 🌟 RECOMMENDED - One key for all models
